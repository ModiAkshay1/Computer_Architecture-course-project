...
class Router:
    ...
    def update(self,instruction, clock_cyle,partname, source, destination,delay_now):
        ...
        if(self.pva):
            ...
            R1 = ["delayed "+str(self.delayed)," Clock cycle: ", str(clock_cyle) + " ||", " Flit: ",partname +" ||"," Source: ", instruction.source + " ||"," Destination: ",instruction.destination + " ||"," Present router: ",self.name+" ||"," Type: "+type_of_element +" ||"," Delay: ",str(delay_now), "\n"]
                ...
                if len(instruction.route) > 1 :
                    ...
                    if instruction.route[0].current_element==2 :
                        ...
                        if len(instruction.route) == 1 and instruction.name==" Tail " :
                            ...
                            if pvs_pva==0:
                                latency_1[key]=delay_now-instruction.start_time+delay_max
                            elif pvs_pva==1:
                                latency_2[key]=delay_now-instruction.start_time+delay_max
