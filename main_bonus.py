...
class Router:
    ...
    def update(self,instruction, clock_cyle,partname, source, destination,delay_now):
        print("rou",instruction.name)
        if (self.current_element==-1):
            queues[self.id].append(instruction)
        # if self.current_element==0 and len(queues[self.id])>1:
        #     if queues[self.id][0]!=instruction:
        #         return
        #     else:
        #         queues[self.id].pop(0)
        # elif self.current_element==0 and len(queues[self.id])==1:
        #     queues[self.id].pop(0)
        print("rou",instruction.name)
...
class NoC:
    ...
    all_instructions=[]
    clk1 = 15 #iniitiating with 50 total cycles and increase if an instruction comes with clock cycle>50
    ...
    def find_instructions(self, clock_cycle):
        ...
        if len(self.traffic[0])>0:
            tmp=[]
            for i in range(len(self.traffic[0])):
                if int(self.traffic[0][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[0][i])
                    tmp.append(self.traffic[0][i])
            for j in tmp:
                self.traffic[0].pop(self.traffic[0].index(j))
        if len(self.traffic[1])>0:
            tmp=[]
            for i in range(len(self.traffic[1])):
                if int(self.traffic[1][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[1][i])
                    tmp.append(self.traffic[1][i])
            for j in tmp:
                self.traffic[1].pop(self.traffic[1].index(j))
        if len(self.traffic[2])>0:
            tmp=[]
            for i in range(len(self.traffic[2])):
                if int(self.traffic[2][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[2][i])
                    tmp.append(self.traffic[2][i])
            for j in tmp:
                self.traffic[2].pop(self.traffic[2].index(j))
        if len(self.traffic[3])>0:
            tmp=[]
            for i in range(len(self.traffic[3])):
                if int(self.traffic[3][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[3][i])
                    tmp.append(self.traffic[3][i])
            for j in tmp:
                self.traffic[3].pop(self.traffic[3].index(j))
        if len(self.traffic[4])>0:
            tmp=[]
            for i in range(len(self.traffic[4])):
                if int(self.traffic[4][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[4][i])
                    tmp.append(self.traffic[4][i])
            for j in tmp:
                self.traffic[4].pop(self.traffic[4].index(j))
        if len(self.traffic[5])>0:
            tmp=[]
            for i in range(len(self.traffic[5])):
                if int(self.traffic[5][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[5][i])
                    tmp.append(self.traffic[5][i])
            for j in tmp:
                self.traffic[5].pop(self.traffic[5].index(j))
        if len(self.traffic[6])>0:
            tmp=[]
            for i in range(len(self.traffic[6])):
                if int(self.traffic[6][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[6][i])
                    tmp.append(self.traffic[6][i])
            for j in tmp:
                self.traffic[6].pop(self.traffic[6].index(j))
        if len(self.traffic[7])>0:
            tmp=[]
            for i in range(len(self.traffic[7])):
                if int(self.traffic[7][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[7][i])
                    tmp.append(self.traffic[7][i])
            for j in tmp:
                self.traffic[7].pop(self.traffic[7].index(j))
        if len(self.traffic[8])>0:
            tmp=[]
            for i in range(len(self.traffic[8])):
                if int(self.traffic[8][i].clock_cycle) == clock_cycle:
                    list.append(self.traffic[8][i])
                    tmp.append(self.traffic[8][i])
            for j in tmp:
                self.traffic[8].pop(self.traffic[8].index(j))
    ...
    def stage(self,rout,pvs_pva):
        ...
        print(len(self.all_instructions))
        for clock_cycle in range(clk_total):
            ...
            if len(x) > 0:
                for i in x:
                    queue.append(i)
            print(clock_cycle,len(queue),queues)
            temp=[]
            for instruction in queue:
                ...
                if len(instruction.route) > 1 :
                    ...
                    if instruction.route[0].current_element==0 and len(queues[instruction.route[0].id])>1:
                        if queues[instruction.route[0].id][0]!=instruction:
                            continue
                        else:
                            temp.append(instruction.route[0].id)
                    elif instruction.route[0].current_element==0 and len(queues[instruction.route[0].id])==1:
                        temp.append(instruction.route[0].id)
